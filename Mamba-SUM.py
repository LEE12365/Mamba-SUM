...
class CMTAttention(nn.Module):
    def __init__(self, dim, num_heads, match_factor=4, ffn_expansion_factor=1, scale_factor=8, bias=True,
...
class HFEBlock(nn.Module):
    def __init__(self, dim=48, num_heads=1, match_factor=4, ffn_expansion_factor=1, bias=True, attention_matching=True,
                 ffn_matching=True, ffn_restormer=False):
...
class DownFRG(nn.Module):
    def __init__(self, dim, n_l_blocks=1, n_h_blocks=1, expand=2):
        super().__init__()
        self.dwt = DWT()
        self.l_conv = nn.Conv2d(dim * 3, dim, 3, 1, 1)
        ...
        self.h_fusion_mul = SKFF(dim, height=4, reduction=8)
        self.h_blk = nn.Sequential(*[HFEBlock(dim, match_factor=1, ffn_expansion_factor=1) for _ in range(n_h_blocks)])

    def forward(self, x, x_d,y):
        x_LL, x_HL, x_LH, x_HH = self.dwt(x)
        y_LL, y_HL, y_LH, y_HH = self.dwt(y)

        b, c, h, w = x_LL.shape
        x_LL = self.l_conv(torch.cat([x_LL, x_d,y_LL], dim=1))
        ...
        y_h = self.h_fusion([y_HL, y_LH, y_HH])
        x_h = self.h_fusion_mul([x_HL, x_LH, x_HH,y_h])
...
class WaveMamba(BaseModel):
    def __init__(self, in_chn=3, wf=48, n_l_blocks=[1, 1, 2], n_h_blocks=[1, 1, 1], ffn_scale=2):
        ...
        ### for CT ####
        self.down1 = Down(in_chn, wf)
        self.down2 = Down(wf, wf)
        self.down3 = Down(wf, wf)
    ...
    def forward(self, x,y): # x: Low-dose PET y:CT
        img = x
        ...
        ######## for CT ########
        yimg_down1 = self.conv_01(y)
        yimg_down2 = self.down2(yimg_down1)
        yimg_down3 = self.down3(yimg_down2)


        ######## UNet-64 ########
        # Down-path (Encoder)
        x_l, x_H1 = self.down_group1(x1, img_down1, yimg_down1)
        x_l, x_H2 = self.down_group2(x_l, img_down2,yimg_down2)
        x_l, x_H3 = self.down_group3(x_l, img_down3,yimg_down3)
...
#
#         return x_l
# class UNet_w(BaseModel):
#     def __init__(self, in_chn=3, wf=48, n_l_blocks=[1,1,2], n_h_blocks=[1,1,1], ffn_scale=2):
#         super(UNet_w, self).__init__()
#         self.ps_down1 = nn.Sequential(
#             nn.PixelUnshuffle(2),
#             nn.Conv2d((2**2) * in_chn, wf, 1, 1, 0)
#         )
